...
resultFileName = "2020Nov_Gz.mat";

%% Read Measured Data

% Dataset for Gz direction (Transverse slices), 1mm thickness, 34 mm slice gap, 5 us dwell time
fpath_mat = '../Data20201106/'; 
fn_mat_Slice1 = 'SignalTraH17.mat';
fn_mat_Slice2 = 'SignalTraF17.mat';
fn_mat_Slice1_ref = 'RefTraH17.mat';
fn_mat_Slice2_ref = 'RefTraF17.mat';

% % Dataset for Gx direction (Sagittal slices), 1mm thickness, 34 mm slice gap, 5 us dwell time
% fpath_mat = '../Data20201106/'; 
% fn_mat_Slice1 = 'SignalSagL17.mat';
% fn_mat_Slice2 = 'SignalSagR17.mat';
% fn_mat_Slice1_ref = 'RefSagL17.mat';
% fn_mat_Slice2_ref = 'RefSagR17.mat';

% % Dataset for Gy direction (Coronal slices), 1mm thickness, 34 mm slice gap, 5 us dwell time
% fpath_mat = '../Data20201110/'; 
% fn_mat_Slice1 = 'SignalCorP17_5us.mat';
% fn_mat_Slice2 = 'SignalCorA17_5us.mat';
...
if useRefData
    ...
    for nn = 1 : nGradAmp
        if isAvgRepetition
            avgSigS1Corr(:,:,nn) = avgSigS1(:,:,nn) ./ avgRefS1(:,:,floor((nn-1)/2)+1);
            avgSigS2Corr(:,:,nn) = avgSigS2(:,:,nn) ./ avgRefS2(:,:,floor((nn-1)/2)+1);
        else
            avgSigS1Corr(:,:,nn,:) = avgSigS1(:,:,nn,:) ./ avgRefS1(:,:,floor((nn-1)/2)+1,:);
            avgSigS2Corr(:,:,nn,:) = avgSigS2(:,:,nn,:) ./ avgRefS2(:,:,floor((nn-1)/2)+1,:);
        end
...
%     errorBarColor = [128 193 219]/255; % shallow blue
    ...
    hold on;  
    plot(freq(freqDispIndex), GIRF_FT_mean_abs(freqDispIndex), 'b', 'LineWidth', 1);
    xlim(dispFreqRange); ylim([0, 1.1]);
    xlabel('Frequency [kHz]'); ylabel('Magnitude of GIRF [AU]');
    title('Magnitude of GIRF in Frequency Domain');
    hold off;
    ...
    subplot(212);
    plot(freq(freqDispIndex), GIRF_FT_mean_phase(freqDispIndex), 'b', 'LineWidth', 1);
    xlim(dispFreqRange); ylim([-4,4]);
    xlabel('Frequency [kHz]'); ylabel('Phase of GIRF [rad]');
    title('Phase of GIRF in Frequency Domain');
end
